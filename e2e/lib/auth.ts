...
export async function phoneCode(phoneNumber: string) {
  await assertTestDb();
  const [verification] = await db.select().from(schema.verification).where(eq(schema.verification.identifier, phoneNumber)).orderBy(desc(schema.verification.createdAt)).limit(1);
...
export async function signInPhone(context: BrowserContext, phoneNumber: string | null) {
  ...
  expect((await context.request.post(`${apiUrl}/api/auth/phone-number/send-otp`, { headers, data: { phoneNumber } })).ok()).toBeTruthy();
  expect((await context.request.post(`${apiUrl}/api/auth/phone-number/verify`, { headers, data: { phoneNumber, code: await phoneCode(phoneNumber) } })).ok()).toBeTruthy();
}
...
export async function signInPhoneUi(page: Page, phoneNumber: string) {
  ...
  await page.getByRole('textbox', { name: 'OTP digit 1', exact: true }).fill(await phoneCode(phoneNumber));
  await page.getByRole('button', { name: 'Continue', exact: true }).click();
}

export async function emailCode(context: BrowserContext, email: string) {
  const response = await context.request.get(`${providerUrl}/emails?to=${encodeURIComponent(email)}`);
  ...
  if (!last || typeof last !== 'object' || !('html' in last) || typeof last.html !== 'string') throw new Error('Synthetic email OTP was not delivered to provider mailbox');
