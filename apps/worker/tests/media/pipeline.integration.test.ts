...
describe('media pipeline (integration)', () => {
  it('processes a representative image to ready with derivatives + phash', async () => {
    ...
    for (const d of row.derivatives) {
      const key = d.key;
      expect(key).toBe(`derivatives/${projectId}/${imageId}/${d.variant}-wm-v4.${d.format}`);
  ...
  it('refreshes a ready image in place and removes its stale derivatives', async () => {
    ...
    const legacyDerivatives = ready.derivatives.map((derivative) => ({
      ...derivative,
      key: derivative.key.replace('-wm-v4.', '.'),
    }));
    ...
    expect(refreshed.derivatives.every((derivative) => derivative.key.includes('-wm-v4.'))).toBe(
      true,
    );
