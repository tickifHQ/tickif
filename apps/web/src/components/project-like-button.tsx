...
export function ProjectLikeButton({
  ...
  useEffect(() => {
    ...
    let ignore = false;
    let updated = false;
    const unsubscribe = subscribeProjectLikeState(identity, projectId, (nextState) => {
      updated = true;
      setResolved({ key: stateKey, state: nextState, error: null });
    });
    async function loadState() {
      try {
        const loaded = await loadProjectLikeState(identity, projectId);
        if (!ignore && !updated) setResolved({ key: stateKey, state: loaded, error: null });
      } catch (error) {
        if (!ignore && !updated)
          setResolved({
            key: stateKey,
  ...
  }, [identity, projectId, stateKey, sessionPending, attempt]);
  ...
  async function toggleLike() {
    ...
    if (session.session.activeOrganizationId) {
      setResolved({
        key: stateKey,
        state,
        error: 'Switch to your personal account to like projects.',
      });
      return;
    }
    ...
    try {
      ...
      if (response.status === 403)
        throw new Error('Likes require an active account in personal context.');
