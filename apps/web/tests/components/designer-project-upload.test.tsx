...
describe('DesignerProjectUpload', () => {
  ...
  it('lets the designer enter a custom city that is not in the taxonomy list', async () => {
    const user = userEvent.setup();
    const { container } = render(<DesignerProjectUpload />);
    ...
    // Switching to the custom-city affordance replaces the dropdown with a
    // free-text input and clearly indicates the custom mode.
    await user.click(screen.getByRole('button', { name: /enter a custom city/i }));
    const customCityInput = screen.getByLabelText('City') as HTMLInputElement;
    expect(customCityInput.tagName).toBe('INPUT');
    expect(screen.getByText(/entering a custom city not in the list/i)).toBeInTheDocument();
    ...
    expect(screen.getByDisplayValue('2 BHK in Pondicherry')).toBeInTheDocument();
  });

  it('restores the taxonomy dropdown when switching back from a custom city', async () => {
    const user = userEvent.setup();
    const { container } = render(<DesignerProjectUpload />);
    ...
    // The taxonomy dropdown is back and the free-text custom-city hint is gone.
    expect(selectWithOption(container, 'Chennai')).toBeInTheDocument();
    expect(screen.queryByText(/entering a custom city not in the list/i)).not.toBeInTheDocument();
  });

  it('loads an existing project saved with a custom city in custom mode', async () => {
    const response = await mock.projectGet();
    const project = (await response.json()) as Record<string, unknown>;
    mock.projectGet.mockResolvedValue(
      new Response(
    ...
    expect(cityInput.value).toBe('Pondicherry');
    expect(screen.getByText(/entering a custom city not in the list/i)).toBeInTheDocument();
  });
  ...
  it('associates project metadata labels with their form controls', async () => {
    ...
    expect(await screen.findByLabelText('Project name')).toHaveValue('2 BHK in Adyar');
    expect(screen.getByLabelText('Apartment / Building name')).toHaveValue('Maitri Apartments');
    expect(screen.getByRole('img', { name: 'Maitri Apartments logo' })).toBeInTheDocument();
    expect(screen.getByLabelText('About the project')).toHaveValue('');
  });

  it('shows the matching logo when a configured apartment is selected', async () => {
    const user = userEvent.setup();
    render(<DesignerProjectUpload />);

    const apartmentInput = await screen.findByRole('combobox', {
      name: 'Apartment / Building name',
    });
    await user.click(apartmentInput);

    const option = screen.getByRole('option', { name: /prestige lakeside/i });
    expect(within(option).getByRole('img', { name: 'Prestige Lakeside logo' })).toBeInTheDocument();

    await user.click(option);

    expect(apartmentInput).toHaveValue('Prestige Lakeside');
    expect(screen.getByRole('img', { name: 'Prestige Lakeside logo' })).toBeInTheDocument();
  });

  it('supports keyboard selection from the configured apartment list', async () => {
    const user = userEvent.setup();
    render(<DesignerProjectUpload />);
    ...
    await user.type(apartmentInput, 'Casa');

    expect(screen.queryByText(/keep this custom apartment name/i)).toBeNull();
    await user.keyboard('{Enter}');

    expect(apartmentInput).toHaveValue('Casagrand First City');
    expect(screen.getByRole('img', { name: 'Casagrand First City logo' })).toBeInTheDocument();
  });

  it('keeps custom apartment names editable without showing a mismatched logo', async () => {
    const user = userEvent.setup();
    render(<DesignerProjectUpload />);
    ...
    await user.type(apartmentInput, 'Independent Residency');

    expect(apartmentInput).toHaveValue('Independent Residency');
    expect(screen.getByText(/keep this custom apartment name/i)).toBeInTheDocument();
    expect(screen.queryByRole('img', { name: /independent residency logo/i })).toBeNull();
  });

  it('keeps the apartment menu inside a narrow mobile viewport', async () => {
    const widthSpy = vi.spyOn(window, 'innerWidth', 'get').mockReturnValue(320);
    const heightSpy = vi.spyOn(window, 'innerHeight', 'get').mockReturnValue(568);
    ...
    try {
      ...
      expect(screen.getByRole('listbox')).toHaveStyle({ left: '8px', width: '304px' });
    } finally {
      rectSpy.mockRestore();
  ...
  it('keeps the standard building-name field for non-apartment projects', async () => {
    const user = userEvent.setup();
    render(<DesignerProjectUpload />);
    ...
    const buildingInput = screen.getByLabelText('Villa / Building name');
    expect(buildingInput).toHaveAttribute('type', 'text');
    expect(buildingInput).not.toHaveAttribute('role', 'combobox');
  });
