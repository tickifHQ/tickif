...
describe('project likes routes', () => {
  it('persists idempotent likes separately from bookmarks and exposes no identities', async () => {
    ...
    for (let index = 0; index < 2; index++) {
      const response = await app.request(`/api/project-likes/${project.id}`, {
        method: 'PUT',
    ...
    for (const [cookie, liked] of [
      [visitor.cookie, true],
      [other.cookie, false],
      ['', false],
    ] as const) {
      const response = await app.request(`/api/project-likes/state?projectIds=${project.id}`, {
        headers: { cookie },
      });
      expect(response.status).toBe(200);
      expect(response.headers.get('cache-control')).toBe('private, no-store');
      expect(await response.json()).toEqual({
        projects: [{ projectId: project.id, liked, likeCount: 1 }],
      });
    }
  ...
  it('serializes concurrent retries and different visitors without losing counts', async () => {
    ...
    expect(
      results.every((state) => state?.liked && state.likeCount >= 1 && state.likeCount <= 5),
    ).toBe(true);
    expect(await projectLikesRepository.state(null, [project.id])).toEqual([
      { projectId: project.id, liked: false, likeCount: 5 },
    ]);
    await Promise.all(
      visitors.flatMap((visitor) => [
    ...
    expect(await projectLikesRepository.state(null, [project.id])).toEqual([
      { projectId: project.id, liked: false, likeCount: 0 },
    ]);
  });
  ...
  it('hides unpublished projects and inactive designers on both reads and writes', async () => {
    ...
    for (const reason of ['unpublished', 'inactive'] as const) {
      await db
        ...
        .set({ status: reason === 'unpublished' ? 'draft' : 'published' })
        .where(eq(schema.project.id, project.id));
      await db
        .update(schema.designerProfile)
        .set({ status: reason === 'inactive' ? 'suspended' : 'active' })
      ...
      const read = await app.request(`/api/project-likes/state?projectIds=${project.id}`, {
        headers: { cookie: visitor.cookie },
      });
  ...
  it('guards mutations, validates input, and rejects banned sessions', async () => {
    ...
    for (const method of ['PUT', 'DELETE']) {
      ...
      expect(
        (
          await app.request('/api/project-likes/not-a-uuid', {
            method,
            headers: { cookie: visitor.cookie },
          })
        ).status,
    ...
    for (const method of ['PUT', 'DELETE']) {
      expect(
        (
          await app.request(`/api/project-likes/${project.id}`, {
            method,
            headers: { cookie: visitor.cookie },
          })
        ).status,
  ...
  it('requires personal context for organization members', async () => {
    ...
    for (const method of ['PUT', 'DELETE']) {
      ...
      expect(await response.json()).toMatchObject({
        error: { message: 'Switch to personal context to continue' },
      });
    }
  });
  ...
  it('cascades likes when a visitor or project is deleted', async () => {
    ...
    expect(await projectLikesRepository.state(null, [project.id])).toEqual([
      { projectId: project.id, liked: false, likeCount: 1 },
    ]);
