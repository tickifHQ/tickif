...
import {
  errorResponseSchema,
  projectLikeParamSchema,
  projectLikeStateSchema,
  projectLikesStateQuerySchema,
  projectLikesStateResponseSchema,
} from '@repo/contracts';
...
const stateRoute = createRoute({
  ...
  responses: {
    200: {
      description: 'Visible project like states',
      content: { 'application/json': { schema: projectLikesStateResponseSchema } },
    },
    422: errorJson('Invalid project ids'),
  },
});

const mutationRoute = <Method extends 'put' | 'delete'>(method: Method) =>
  createRoute({
    method,
    path: '/{projectId}',
    tags: ['Project Likes'],
    summary:
      method === 'put'
        ? 'Like a public project idempotently'
        : 'Unlike a public project idempotently',
    security: [{ cookieAuth: [] }],
    middleware: [requireAuth, requirePersonalContext] as const,
    request: { params: projectLikeParamSchema },
    responses: {
      200: {
        description: 'Current project like state',
        content: { 'application/json': { schema: projectLikeStateSchema } },
      },
...
export const projectLikesRoutes = new OpenAPIHono<{ Variables: AuthVariables }>({
  ...
  .openapi(mutationRoute('put'), async (c) => {
    c.header('Cache-Control', 'private, no-store');
    return c.json(
      await projectLikesService.setLiked(
        callerId(c.get('user')),
