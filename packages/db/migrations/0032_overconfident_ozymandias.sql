...
CREATE TABLE "interaction_event" (
	...
	"event_day" date DEFAULT (now() at time zone 'utc')::date NOT NULL,
	...
	CONSTRAINT "interaction_event_day_check" CHECK ("interaction_event"."event_day" = ("interaction_event"."created_at" at time zone 'utc')::date)
);
...
CREATE UNIQUE INDEX "interaction_event_actor_project_day_uniq" ON "interaction_event" USING btree ("actor_user_id","project_id","event_day") WHERE "interaction_event"."type" = 'project_view' AND "interaction_event"."actor_user_id" IS NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX "interaction_event_actor_profile_day_uniq" ON "interaction_event" USING btree ("actor_user_id","designer_profile_id","event_day") WHERE "interaction_event"."type" = 'profile_view' AND "interaction_event"."actor_user_id" IS NOT NULL;--> statement-breakpoint
...
CREATE INDEX "interaction_event_created_idx" ON "interaction_event" USING btree ("created_at");