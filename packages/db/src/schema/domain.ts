...
import { INTERACTION_EVENT_TYPE_VALUES } from '@repo/contracts';
 ...
 * `anonymousId` is an opaque client UUID carried into the authenticated event so
 * later phases can stitch pre/post-login activity. It must never contain a session
 * token, fingerprint, or other personal data.
 */
export const interactionEvent = pgTable(
  ...
  (t) => [
    ...
    index('interaction_event_actor_created_idx').on(t.actorUserId, t.createdAt),
    index('interaction_event_anonymous_created_idx').on(t.anonymousId, t.createdAt),
  ],
);
