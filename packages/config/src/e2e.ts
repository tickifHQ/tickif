...
const localUrl = z.url().refine((value) => ['localhost', '127.0.0.1'].includes(new URL(value).hostname), 'E2E services must use loopback hosts');
export const e2eEnvironmentSchema = z.object({
  NODE_ENV: z.enum(['development', 'test']).default('test'),
  DATABASE_URL_TEST: localUrl.refine((value) => /^\/[a-z0-9_]+_test$/.test(new URL(value).pathname), 'E2E database must end in _test').default('postgresql://tickif:tickif@localhost:5432/tickif_stage12_test'),
  REDIS_URL_TEST: localUrl.refine((value) => /^\/(?:[1-9]|1[0-5])$/.test(new URL(value).pathname), 'E2E Redis needs a dedicated nonzero database').default('redis://localhost:6379/12'),
  TYPESENSE_HOST: localUrl.default('http://localhost:8108'),
  TYPESENSE_API_KEY: z.string().min(16).default('tickif-local-typesense-key'),
  TYPESENSE_COLLECTION_PREFIX: z.string().regex(/^tickif_(?:stage\d+|e2e)(?:_[a-z0-9]+)*$/).default('tickif_e2e'),
  ...
  R2_BUCKET: z.string().regex(/^tickif-(?:stage\d+|e2e)(?:-[a-z0-9]+)*$/).default('tickif-e2e'),
...
export function buildE2eEnvironment(environment: NodeJS.ProcessEnv) {
  ...
  if (!result.success) throw new Error(`Unsafe E2E environment: ${result.error.issues.map((issue) => `${issue.path.join('.')}: ${issue.message}`).join('; ')}`);
  const values = result.data;
  if (environment.DATABASE_URL && environment.DATABASE_URL !== values.DATABASE_URL_TEST) throw new Error('E2E DATABASE_URL must match DATABASE_URL_TEST');
  const api = new URL(values.BETTER_AUTH_URL);
  const web = new URL(values.PUBLIC_WEB_URL);
  if (api.protocol !== 'http:' || web.protocol !== 'http:' || api.hostname !== web.hostname) throw new Error('Local E2E API and web require HTTP on the same loopback hostname');
  const ports = [Number(api.port || 80), Number(web.port || 80), values.WORKER_HEALTH_PORT, values.E2E_PROVIDER_PORT];
  if (new Set(ports).size !== ports.length) throw new Error('E2E API, web, worker and provider ports must differ');
  const serialized = Object.fromEntries(Object.entries(values).map(([key, value]) => [key, String(value)])) as { [K in keyof typeof values]: string };
  return {
    ...environment,
    ...serialized,
    NODE_ENV: 'test',
